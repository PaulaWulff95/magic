...
class Surf:
    ...
    def surf(self, field='Bphi', proj='hammer', lon_0=0., r=0.85, vmax=None,
        ...
        if field in ('Vs', 'vs'):
            ...
            data = -rderavg(self.gr.Br**2+self.gr.Btheta**2+self.gr.Bphi**2, self.gr.radius)/2.0
            data_ic = None
    ...
    def equat(self, field='vr', levels=defaultLevels, cm=None,
        ...
        if field in ('vortzfluct', 'wzfluct'):
            ...
            dr = rderavg(rrloc*vpfluct[:,self.gr.ntheta//2,:], self.gr.radius, exclude=True)
            ...
            philoc = np.linspace(0., 2.*np.pi/self.gr.minc, self.gr.npI)
            rrloc, pphiloc = np.meshgrid(self.gr.radius, philoc)
            dr = rderavg(rrloc*self.gr.vphi[:,self.gr.ntheta//2,:], self.gr.radius, exclude=True)
            ...
            philoc = np.linspace(0., 2.*np.pi/self.gr.minc, self.gr.npI)
            rrloc, pphiloc = np.meshgrid(self.gr.radius, philoc)
            dr = rderavg(rrloc*self.gr.Bphi[:,self.gr.ntheta//2,:], self.gr.radius, exclude=True)
    ...
    def avg(self, field='vphi', levels=defaultLevels, cm=None,
        ...
        if field not in ('Cr', 'cr', 'ra', 'ratio', 'Cz', 'cz', 'hz', 'jphi',
            ...
            phiavg = phiavg = zderavg(data.mean(axis=0), self.gr.radius, exclude=True)

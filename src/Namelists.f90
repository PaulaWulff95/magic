...
contains
   ...
   subroutine readNamelists(tscheme)
      ...
      &    epsPhase,ktopphi,kbotphi,aCond,bCond,rSSL
      ...
      if ( imagcon /= 0 .and. tmagcon == 0 ) tmagcon=0!1.0e18_cp    CHANGED BY PW
      ...
      if ( l_mag_nl .and. kbotv == 1 .and. l_cond_ic ) then
         if ( rank == 0 ) then
            ...
            write(output_unit,*) '! Is this the problem?'
         end if
         l_b_nl_icb=.true.
      end if
      ...
      !-- Special matrix for b(l=1,m=0) if axial dip. at icb & conducting IC:
      l_b10mat=.false.
      if ( imagcon == -1 .and. kbotb == 3 ) l_b10mat= .true.
...
!--------------------------PW
      write(n_out,'(''  aCond   ='',ES14.6,'','')') aCond
      write(n_out,'(''  bCond   ='',ES14.6,'','')') bCond
      write(n_out,'(''  rSSL   ='',ES14.6,'','')') rSSL
!--------------------------PW

module updateB_mod
   ...
   use logic, only: l_cond_ic, l_LCR, l_rot_ic, l_mag_nl, l_b_nl_icb, &
       ...
       &            l_full_sphere, l_mag_par_solve, l_b10Mat
   ...
   subroutine initialize_updateB()
      ...
      if ( .not. l_mag_par_solve ) then
         ...
         if ( l_finite_diff ) then
            ...
            allocate( type_densemat :: b10Mat )

            call b10Mat%initialize(n_r_max,n_r_max,l_pivot=.true.)
            do ll=1,nLMBs2(1+rank)
...
!-----------------------------------------------------------------------------
   subroutine finalize_updateB()
      ...
      if ( .not. l_mag_par_solve ) then
         ...
         call b10Mat%finalize()

         if ( l_cond_ic ) deallocate ( work_ic_LMloc )

#ifdef WITH_PRECOND_BJ
         deallocate(bMat_fac,jMat_fac, b10Mat_fac)
#endif
...
!-----------------------------------------------------------------------------
   subroutine updateB(b,db,ddb,aj,dj,ddj,dbdt,djdt,b_ic,db_ic,ddb_ic,aj_ic,  &
      ...
      complex(cp) :: rhs(n_r_max)   ! RHS of matrix multiplication, just for imagcon=-1 and cond_ic
      integer :: nR                  ! No of radial grid point
!      logical :: l_b10Mat            !do we need to solve the 1,0 mode differently?
      ...
      do nLMB2=1,nLMBs2(nLMB)
         ...
         if ( l1 > 0 ) then
            ...
            if ( l_b10mat .and. l1==1 ) then
               !----- Special treatment of b10 component if conducting ic and
               !      holding m=0, l=1 mode at icb (l_b10mat=.true.)
               if ( .not. lB10mat ) then
#ifdef WITH_PRECOND_BJ
                   call get_b10Mat(tscheme,l1,hdif_B(l1),b10Mat, b10Mat_fac)
#else
                   call get_b10Mat(tscheme,l1,hdif_B(l1),b10Mat)
#endif
                   lB10mat=.true.
               end if
            end if
            ...
            do iChunk=1,nChunks
               ...
               do lm=lmB0+1,min(iChunk*chunksize,sizeLMB2(nLMB2,nLMB))
                  ...
                  if ( m1 == 0 ) then   ! Magnetoconvection boundary conditions
                     !write(*,*) "tmagcon", tmagcon
!                     if ( imagcon /= 0 .and. tmagcon <= time ) then
                     if ( imagcon /= 0) then
!                        if ( l_LCR ) then
                        if ( l_LCR .and. imagcon /= -1) then
            ...
            if ( lmB > lmB0 ) then
               ...
               if (l_b10Mat) then
                  rhs(:) = rhs1(1:n_r_max, 2*(lmB0+1)-1, threadid)
                  !write(*,*) 'rhs', rhs
               end if
#ifdef WITH_PRECOND_BJ
               if (l_b10Mat) then
            ...
            if ( lRmsNext .and. tscheme%istage == 1 ) then ! Store old b,aj
               ...
               do lm=lmB0+1,min(iChunk*chunksize,sizeLMB2(nLMB2,nLMB))
                  ...
                  if ( m1 > 0 ) then
                     ...
                     !if ( l1 == 1 ) write(*,*) 'lm1 for m=0, l=1', lm1
                     do n_r_out=1,rscheme_oc%n_max   ! outer core
                        b(lm1,n_r_out) = cmplx(rhs1(n_r_out,2*lm-1,threadid), &
                        &                      0.0_cp,kind=cp)
                        aj(lm1,n_r_out)= cmplx(rhs2(n_r_out,2*lm-1,threadid), &
                     ...
                     if ( l_b10Mat .and. l1 == 1) then
                        b(lm1, :) = cmplx(real(rhs(:)), 0.0_cp, kind=cp)
                        b_ic(lm1, :) = zero
                     end if
!                     write (*,*) 'b2', b(121, :)
                  end if
...
!-----------------------------------------------------------------------------
#ifdef WITH_PRECOND_BJ
   subroutine get_b10Mat(tscheme,l,hdif,b10Mat,b10Mat_fac)
#else
   subroutine get_b10Mat(tscheme,l,hdif,b10Mat)
#endif
      !
      !  Purpose of this subroutine is to contruct the time step matrix
      !  ``b10mat(i,j)`` for the poloidal magnetic potential b of degree
      !  :math:`\ell=1` and order :math:`m=0`.
      !  This differs from the the normal bmat only if we implement an
      !  axial dipole at the icb and have a conducting core.
      !  (``kbotb=3`` and ``imagcon=-1``).
      !
      ...
      !-- Output variables:
      class(type_realmat), intent(inout) :: b10Mat
#ifdef WITH_PRECOND_BJ
      real(cp), intent(out) :: b10Mat_fac(n_r_maxMag)
#endif
      ...
      real(cp) :: dLh
      real(cp) :: rRatio
      real(cp) :: datBmat(n_r_max,n_r_max)
...
!      if ( l_cond_ic ) nRall=nRall+n_r_ic_max
      ...
      !-------- Imposed fields axial dipole at ri: (overwrites above IC boundary cond.)
      datBmat(n_r_max,1:n_r_max)=rscheme_oc%rnorm * rscheme_oc%rMat(n_r_max,:)
      ...
      ! compute the linesum of each line
      do nR=1,nRall
         b10Mat_fac(nR)=one/maxval(abs(datBmat(nR,1:nRall)))
         datBmat(nR,:) = datBmat(nR,:)*b10Mat_fac(nR)
      end do
#endif
...
#undef MATRIX_CHECK
      ...
      ! copy the bMat to a temporary variable for modification
      write(filename,"(A,I3.3,A,I3.3,A)") "b10Mat_",l,"_",counter,".dat"
      open(newunit=filehandle,file=trim(filename))
      counter= counter+1
      ...
      ! estimate the condition number
      call dgecon('I',n_r_tot,temp_Mat,n_r_tot,anorm,rcond,work,iwork,info)
      write(*,"(A,I3,A,ES11.3)") "inverse condition number of b10Mat for l=",l," is ",rcond

      end block
#endif

      !-- Array copy
      call b10Mat%set_data(datBmat)

      !----- LU decomposition:
      call b10Mat%prepare(info)
      if ( info /= 0 ) call abortRun('Singular matrix bMat in get_bmat')

   end subroutine get_b10Mat
!-----------------------------------------------------------------------------
